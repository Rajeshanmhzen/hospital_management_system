...
type SortBy = "name" | "monthlyPrice" | "yearlyPrice" | "displayOrder" | "createdAt";
...
const PricingPlanPage = () => {
    ...
    const fetchPlans = async () => {
        setLoading(true);
        try {
            const response = await api.get<PricingPlan[]>("/pricing-plans/admin/list");
            setPlans(Array.isArray(response.data) ? response.data : []);
        } catch (error: any) {
            notifications.show({
                title: "Failed",
                message: error?.response?.data?.message || "Unable to fetch pricing plans",
                color: "red",
            });
        } finally {
            setLoading(false);
    ...
    const pageItems = filteredSortedPlans.slice((page - 1) * pageSize, page * pageSize);
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            notifications.show({
                title: "Update Failed",
                message: error?.response?.data?.message || "Unable to update pricing plan",
                color: "red",
            });
        } finally {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            notifications.show({
                title: "Delete Failed",
                message: error?.response?.data?.message || "Unable to delete pricing plan",
                color: "red",
            });
        } finally {
            ...
            render: (plan) => (
                <Group gap="xs" justify="center">
                    <Tooltip label="View" withArrow>
                        <ActionIcon variant="light" color="blue" onClick={() => handleView(plan)}>
                            <IconEye size={16} />
                        </ActionIcon>
                    </Tooltip>
                    <Tooltip label="Edit" withArrow>
                        <ActionIcon variant="light" color="yellow" onClick={() => handleEdit(plan)}>
                            <IconEdit size={16} />
                        </ActionIcon>
                    </Tooltip>
                    <Tooltip label="Delete" withArrow>
                        <ActionIcon variant="light" color="red" onClick={() => handleDelete(plan)}>
                            <IconTrash size={16} />
                        </ActionIcon>
                    </Tooltip>
                </Group>
    ...
    return (
        <Container size="xl" py="xl">
            <Stack gap="xl">
                <Group align="center" wrap="nowrap" justify="space-between">
                    <Box>
                        ...
                        <Text c="dimmed" size="md" fw={500}>Manage your pricing plans and their information.</Text>
                    </Box>
                    <Group>
                        <Button leftSection={<IconDownload size={14} />} variant="outline" onClick={exportCsv}>
                            Export Plans
                        </Button>
                ...
                <Paper withBorder radius="md" p="md">
                    <Group justify="space-between" align="end" mb="md" wrap="wrap">
                        ...
                        <Select
                            label="Status"
                            value={statusFilter}
                            onChange={(value) => setStatusFilter((value as PlanStatusFilter) || "ALL")}
                            data={[
                                { value: "ALL", label: "All" },
            ...
            >
                {selectedPlan ? (
                    <Stack>
                        <Text><b>Name:</b> {selectedPlan.name}</Text>
                        <Text><b>Description:</b> {selectedPlan.description}</Text>
                        <Text><b>Monthly:</b> ${Number(selectedPlan.monthlyPrice).toFixed(2)}</Text>
                        <Text><b>Yearly:</b> ${Number(selectedPlan.yearlyPrice).toFixed(2)}</Text>
                        <Text><b>Max Users:</b> {selectedPlan.maxUsers}</Text>
                        <Text><b>Max Patients:</b> {selectedPlan.maxPatients}</Text>
                        <Text><b>Features:</b> {Array.isArray(selectedPlan.features) ? selectedPlan.features.join(", ") : "-"}</Text>
                        <Text><b>Display Order:</b> {selectedPlan.displayOrder}</Text>
                    </Stack>
                ) : (
            ...
            >
                <Stack>
                    <TextInput label="Plan Name" value={editForm.name} onChange={(e) => setEditForm((p) => ({ ...p, name: e.currentTarget.value }))} />
                    <Textarea label="Description" value={editForm.description} onChange={(e) => setEditForm((p) => ({ ...p, description: e.currentTarget.value }))} minRows={2} />
                    <Group grow>
                        <NumberInput label="Monthly Price" value={editForm.monthlyPrice} onChange={(v) => setEditForm((p) => ({ ...p, monthlyPrice: Number(v) || 0 }))} />
                        <NumberInput label="Yearly Price" value={editForm.yearlyPrice} onChange={(v) => setEditForm((p) => ({ ...p, yearlyPrice: Number(v) || 0 }))} />
                    </Group>
                    <Group grow>
                        <NumberInput label="Max Users" value={editForm.maxUsers} onChange={(v) => setEditForm((p) => ({ ...p, maxUsers: Number(v) || 0 }))} />
                        <NumberInput label="Max Patients" value={editForm.maxPatients} onChange={(v) => setEditForm((p) => ({ ...p, maxPatients: Number(v) || 0 }))} />
                    </Group>
                    <TextInput
                        label="Features"
                        value={editForm.featuresText}
                        onChange={(e) => setEditForm((p) => ({ ...p, featuresText: e.currentTarget.value }))}
                    />
                    <Group grow>
                        <Switch label="Active" checked={editForm.isActive} onChange={(e) => setEditForm((p) => ({ ...p, isActive: e.currentTarget.checked }))} />
                        <Switch label="Public" checked={editForm.isPublic} onChange={(e) => setEditForm((p) => ({ ...p, isPublic: e.currentTarget.checked }))} />
                    </Group>
                    <NumberInput label="Display Order" value={editForm.displayOrder} onChange={(v) => setEditForm((p) => ({ ...p, displayOrder: Number(v) || 0 }))} />
                    <Group justify="flex-end">
                        <Button variant="default" onClick={() => setEditOpened(false)}>Cancel</Button>
                        <Button loading={saveLoading} onClick={handleSaveEdit}>Save Changes</Button>
                    </Group>
                </Stack>
            </EntityDetailsModal>
